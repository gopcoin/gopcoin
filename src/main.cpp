...
#if defined(NDEBUG)
# error "Gopcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x73f0ea4185efa783abcb33ce8ab34f97fddde438c6abffde05b6571dea800ead");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Gopcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Gopcoin Signed Message:\n";
...
{
    // Gopcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Gopcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Gopcoin: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // Gopcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Gopcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Gopcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xfc66fd8bd538d68d055176edb44a91e088449151962b3e961ad8969b9a169bba");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Biboran Ansha Abdul";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1518449256;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085664753;

        if (fTestNet)
        {
            block.nTime    = 1518448781;
            block.nNonce   = 385545099;
        }
	...
	if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x0488fa7a9474123c35576382c2ef44d2f2beebbda15e6e9e1f8886c9f9a4bbb5"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc1, 0xb4, 0xda }; // Gopcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GopcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GopcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GopcoinMiner(CWallet *pwallet)
{
    printf("GopcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("gopcoin-miner");
    ...
    try { loop {
        ...
        printf("Running GopcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GopcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GopcoinMiner, pwallet));
}
